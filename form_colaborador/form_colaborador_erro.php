...
$old_error_handler = set_error_handler("form_colaborador_scriptcase_error_handler");

function form_colaborador_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_colaborador']['contr_erro']) && $_SESSION['scriptcase']['form_colaborador']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_colaborador->contr_form_colaborador->Apl_com_erro = true;
            if ($inicial_form_colaborador->contr_form_colaborador->NM_ajax_flag)
            {
                $inicial_form_colaborador->contr_form_colaborador->NM_ajax_info['errList']['geral_form_colaborador'][] = $msg;
                return;
            }
            form_colaborador_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_colaborador_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_colaborador'])  && '' != $_SESSION['scriptcase']['error_icon']['form_colaborador'])  ? $_SESSION['scriptcase']['error_icon']['form_colaborador']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_colaborador']) && '' != $_SESSION['scriptcase']['error_close']['form_colaborador']) ? $_SESSION['scriptcase']['error_close']['form_colaborador'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_colaborador']) && '' != $_SESSION['scriptcase']['error_span_title']['form_colaborador']) ? $_SESSION['scriptcase']['error_span_title']['form_colaborador'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_colaborador']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_colaborador']) ? $_SESSION['scriptcase']['error_icon_title']['form_colaborador'] : '';
?>
