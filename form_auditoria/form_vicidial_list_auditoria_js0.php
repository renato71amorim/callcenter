<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="lead_id" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['lead_id']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_vicidial_list_auditoria']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo $this->form_encode_input(session_id()); ?>"> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_vicidial_list_auditoria"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_vicidial_list_auditoria']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_vicidial_list_auditoria_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}
...
function cep_postal_code(num_cep, campos_cep)
{
   retorno_cep  = campos_cep;
   jsrsExecute("form_vicidial_list_auditoria_cep.php?cep=" + num_cep + "&onchange=s", recebe_postal_code, "postal_code", num_cep, false);
}
function recebe_postal_code(string)
{
...
{
   nm_field_disabled("modify_date=disabled;user=disabled", "");
